...
QUERY_METADATA_FEMINICIDIOS = [
    ...
    ("Feminicídios Relação de RAIs", "feminicidios_relacao_rais.sql"),
]
...
#rodape kpi feminicidios em mes (mês corrente até agora)
pdf.set_font('Arial', 'I', 8) 
pdf.set_text_color(0, 0, 0)
pdf.set_x(kpi_x)
...
pdf.set_font('Arial', 'B', 12)
pdf.set_text_color(0, 0, 0)
titulo_mes_regiao = f'Feminicídios por dia por Região no mês de referência (até hoje): {mes_atual}/{ano_atual}'
...
if not df_comparativo_dia.empty:
    ...
    try:
        plt.savefig(os.path.join(relatorio_dir, 'grafico_feminicidio_dia_regiao_ate_hoje.png'), dpi=150, bbox_inches='tight')
    except Exception as e:
        print(f"Erro ao salvar gráfico: {e}")
        try:
            plt.savefig(os.path.join(relatorio_dir, 'grafico_feminicidio_dia_regiao_ate_hoje.png'), format='png', dpi=100)
...
grafico_path = os.path.join(relatorio_dir, 'grafico_feminicidio_dia_regiao_ate_hoje.png')
...
pdf.set_font('Arial', 'B', 12)
pdf.set_text_color(0, 0, 0)  # Preto
titulo_mes_atual = f'Feminicídios - Comparativos por dia no mês/ano de referência (até hoje): {mes_atual}/{ano_atual}'
...
pdf.set_font('Arial', 'B', 12)
pdf.set_text_color(0, 0, 0)  # Preto
titulo_por_dia = f'Feminicídios por dia e ano no mês de referência (até hoje) :'
...
# Gera o gráfico comparativo de feminicídios por dia por região
pdf.garantir_espaco_ou_nova_pagina(80)
columns_dia_regioes, rows_dia_regioes = resultados["Feminicídios Comparativo por Dia por Regiões"]
...
pdf.set_font('Arial', 'B', 12)
pdf.set_text_color(0, 0, 0)
titulo_mes_regiao = f'Feminicídios por dia por Região no mês de referência (até ontem): {mes_atual}/{ano_atual}'
pdf.cell(0, 10, titulo_mes_regiao, ln=1, align='L')
...
if not df_comparativo_dia.empty:
    ...
    # Salva o gráfico com tratamento de erro
    try:
        plt.savefig(os.path.join(relatorio_dir, 'grafico_feminicidio_dia_regiao_ate_ontem.png'), dpi=150, bbox_inches='tight')
    except Exception as e:
        print(f"Erro ao salvar gráfico: {e}")
        try:
            plt.savefig(os.path.join(relatorio_dir, 'grafico_feminicidio_dia_regiao_ate_ontem.png'), format='png', dpi=100)
        except Exception as e2:
            print(f"Erro ao salvar com configurações básicas: {e2}")
            plt.figure(figsize=(10, 3.0))
            plt.text(0.5, 0.5, 'Gráfico não disponível', ha='center', va='center', transform=plt.gca().transAxes)
...
grafico_path = os.path.join(relatorio_dir, 'grafico_feminicidio_dia_regiao_ate_ontem.png')
if not df_comparativo_dia.empty and os.path.exists(grafico_path):
    try:
        pdf.image(grafico_path, x=5, w=200)
    except Exception as e:
...
pdf.set_font('Arial', 'B', 12)
pdf.set_text_color(0, 0, 0)
titulo_semana_regiao = f'Feminicídios - dias da semana por Região no ano {hoje.year}:'
